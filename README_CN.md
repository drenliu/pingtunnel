...
基于 ICMP (Ping) 协议的 TCP 端口转发工具。将 TCP 流量封装在 ICMP Echo Request/Reply 数据包中传输，可穿透仅允许 Ping 的网络环境。

## 特性

- **ICMP 隧道** — TCP 流量封装在 Ping 包中，绕过 TCP/UDP 防火墙限制
...
```bash
sudo ./pingtunnel -type client -l <监听地址> -s <服务器IP> -t <目标地址> -key <隧道密钥>
```
...
├── server.go        # 服务器端：ICMP 监听、TCP 转发
├── client.go        # 客户端：ICMP 隧道连接、TCP 转发
...
2. 服务器监听 TCP 端口，接受外部连接
3. TCP 数据封装为 ICMP 数据包，通过隧道传输到客户端
4. 客户端解封装后转发到目标地址
